...
def paytm(request):
    ...
    paytmParams["body"] = {
        ...
        "orderId"       : order_id,
        "callbackUrl"   : "http://127.0.0.1:8000/app/handlerequest/",
        "txnAmount"     : {
            "value"     : amount,
            "currency"  : "INR",
        },
...
def products(request):
    if request.user.is_authenticated:
        if request.user.is_superuser == 0:
            ...
            params = {'products':products, 'productsCount':products.count()}
            return render(request, 'app/product-list.html', params)
...
def productsDetails(request, id):
    ...
    params = {'product':product, 'products':products}
    return render(request, 'app/product-details.html', params)
...
def cart(request):
    if request.method == 'POST':
        ...
        total = qnt * int(product.price)

        Cart(user = user ,product = product, qnt = qnt , total = total).save()
...
def checkout(request):
    ...
    params = {'cart':cart}
    return render(request, 'app/checkout.html' , params)